...
use common\{DatabaseObject, NamedArguments, Configuration, CoralSession};
use resources\controllers\{ResourceAcquisition};
use resources\models as db;
//Status, ResourceType, ResourceFormat, AcquisitionType, ResourceAcquisition, IsbnOrIssn, ResourceRelationship, Alias, ResourceNote, NoteType, ExternalLogin, User, GeneralSubject, DetailedSubject, PurchaseSite, AuthorizedSite, AdministeringSite, AuthenticationType, CatalogingStatus, GeneralDetailSubjectLink
class Resource extends DatabaseObject {
	//*** Search Functions ***//
		...
		public static function setSearch($search) {
			...
			$systemDefaultSort = "R.createDate DESC, TRIM(LEADING 'THE ' FROM UPPER(R.titleText)) ASC";
			$orderBy = ($config->settings->defaultSort) ?? $systemDefaultSort; 
	...
	//*** Querying for Resources ***/
		...
		public function getResourceByTitle($title) {
			$resourceModel = new db\Resource();
			$results = $resourceModel->getResourcesByTitle($title);
			$outputObjects = [];
		...
		public function getResourceByEbscoKbId($ebscoKbId) {
			$resourceModel = new db\Resource();
			$result = $resourceModel->getResourceByEbscoKbID($ebscoKbId);
			$resourceExists = isset($result[0]['resourceID']);
			$output = ($resourceExists) ? new Resource(new NamedArguments(['primaryKey' => $result[0]['resourceID']])) : FALSE;
			return $output;
		}

		public function getResourceAcquisitions() {
			$resourceModel = new db\Resource(new NamedArguments(array('primaryKey' => $this->resourceID)));
			$result = $resourceModel->getResourceAcquisitions();
			$objects = array();
		...
		public function getInitialNote() {
			...
			$initialNoteID = $noteType->getInitialNoteTypeID();

			$resourceModel = new db\Resource(new NamedArguments(array('primaryKey' => $this->resourceID)));
			$result = $resourceModel->getInitialNote($initialNoteID);

			$returnedNoteID = ($result[0]['resourceNoteID']) ?? NULL;
			return new db\ResourceNote(new NamedArguments(['primaryKey' => $returnedNoteID]));
		}

	// Tim Note: Look over and potentially refactor.
	/*
