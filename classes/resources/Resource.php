...
class Resource extends \common\DatabaseObject {
    ...
    public function asArray() {
        ...
        $status = new Status(new \common\NamedArguments(array('primaryKey' => $this->statusID)));
        $rarray['status'] = $status->shortName;

        if ($this->resourceTypeID) {
            $resourceType = new ResourceType(new \common\NamedArguments(array('primaryKey' => $this->resourceTypeID)));
            $rarray['resourceType'] = $resourceType->shortName;
        }

        if ($this->resourceFormatID) {
            $resourceFormat = new ResourceFormat(new \common\NamedArguments(array('primaryKey' => $this->resourceFormatID)));
            $rarray['resourceFormat'] = $resourceFormat->shortName;
        }

        if ($this->acquisitionTypeID) {
            $acquisitionType = new AcquisitionType(new \common\NamedArguments(array('primaryKey' => $this->acquisitionTypeID)));
    ...
    public function getResourceByTitle($title) {
        ...
        foreach ($result as $row) {
            $object = new Resource(new \common\NamedArguments(array('primaryKey' => $row['resourceID'])));
    ...
    public function getResourceByEbscoKbId($ebscoKbId) {
        ...
        if (isset($result['resourceID'])) {
        	return new Resource(new \common\NamedArguments(['primaryKey' => $result['resourceID']]));
		} else {
        	return false;
		}
    }
    ...
    public function getResourceAcquisitions() {
		...
		foreach ($result as $row) {
			$object = new ResourceAcquisition(new \common\NamedArguments(array('primaryKey' => $row['resourceAcquisitionID'])));
			array_push($objects, $object);
		}
		return $objects;
	...
	public function getIsbnOrIssn() {
		...
		foreach ($result as $row) {
			$object = new IsbnOrIssn(new \common\NamedArguments(array('primaryKey' => $row['isbnOrIssnID'])));
			array_push($objects, $object);
		}
    ...
    private function getRelatedResources($key) {
		...
		if (isset($result['resourceRelationshipID'])) {
			$object = new ResourceRelationship(new \common\NamedArguments(array('primaryKey' => $result['resourceRelationshipID'])));
			array_push($objects, $object);
		}else{
			$db = DBService::getInstance();
			foreach ($result as $row) {
				$object = new ResourceRelationship(new \common\NamedArguments(array('primaryKey' => $row['resourceRelationshipID'],'db'=>$db)));
				array_push($objects, $object);
			}
		}
	...
	public function getNotes($tabName = NULL) {
		...
		foreach ($result as $row) {
			$object = new ResourceNote(new \common\NamedArguments(array('primaryKey' => $row['resourceNoteID'])));
			array_push($objects, $object);
		}
	...
	public function getInitialNote() {
		...
		if (isset($result['resourceNoteID'])) {
			return new ResourceNote(new \common\NamedArguments(array('primaryKey' => $result['resourceNoteID'])));
		} else{
	...
	public function getExternalLogins() {
		...
		if (isset($result['externalLoginID'])){ $result = [$result]; }
		foreach ($result as $row) {
			$object = new ExternalLogin(new \common\NamedArguments(array('primaryKey' => $row['externalLoginID'])));
			array_push($objects, $object);
		}
	...
	public static function setSearch($search) {
		$config = new \common\Configuration;
		...
		\common\CoralSession::set('resourceSearch', $search);
	}
	...
	public static function getSearchDetails() {
		...
		if ($search['statusID']) {
			...
			$status = new Status(new \common\NamedArguments(array('primaryKey' => $search['statusID'])));
		...
		if ($search['creatorLoginID']) {
			...
			$createUser = new User(new \common\NamedArguments(array('primaryKey' => $search['creatorLoginID'])));
		...
		if ($search['resourceFormatID']) {
			...
			$resourceFormat = new ResourceFormat(new \common\NamedArguments(array('primaryKey' => $search['resourceFormatID'])));
		...
		if ($search['acquisitionTypeID']) {
			...
			$acquisitionType = new AcquisitionType(new \common\NamedArguments(array('primaryKey' => $search['acquisitionTypeID'])));
      ...
      $endDate = create_date_from_js_format($search['createDateEnd'])->format('Y-m-d');
		...
		if ($search['resourceTypeID'] == 'none') {
			...
			$resourceType = new ResourceType(new \common\NamedArguments(array('primaryKey' => $search['resourceTypeID'])));
		...
		if ($search['generalSubjectID'] == 'none') {
			...
			$generalSubject = new GeneralSubject(new \common\NamedArguments(array('primaryKey' => $search['generalSubjectID'])));
		...
		if ($search['detailedSubjectID'] == 'none') {
			...
			$detailedSubject = new DetailedSubject(new \common\NamedArguments(array('primaryKey' => $search['detailedSubjectID'])));
		...
		if ($search['noteTypeID'] == 'none') {
			...
			$noteType = new NoteType(new \common\NamedArguments(array('primaryKey' => $search['noteTypeID'])));
		...
		if ($search['purchaseSiteID'] == 'none') {
			...
			$purchaseSite = new PurchaseSite(new \common\NamedArguments(array('primaryKey' => $search['purchaseSiteID'])));
		...
		if ($search['authorizedSiteID'] == 'none') {
			...
			$authorizedSite = new AuthorizedSite(new \common\NamedArguments(array('primaryKey' => $search['authorizedSiteID'])));
		...
		if ($search['administeringSiteID'] == 'none') {
			...
			$administeringSite = new AdministeringSite(new \common\NamedArguments(array('primaryKey' => $search['administeringSiteID'])));
		...
		if ($search['authenticationTypeID'] == 'none') {
			...
			$authenticationType = new AuthenticationType(new \common\NamedArguments(array('primaryKey' => $search['authenticationTypeID'])));
		...
		if ($search['catalogingStatusID'] == 'none') {
			...
			$catalogingStatus = new CatalogingStatus(new \common\NamedArguments(array('primaryKey' => $search['catalogingStatusID'])));
	...
	private static function addIdsToResourcesRow($row) {
		$resource = new Resource(new \common\NamedArguments(array('primaryKey' => $row['resourceID'])));
...
// TODO: i18n; remove other articles
	...
	public function removeResourceAndChildren() {
		...
		foreach ($this->getChildResources() as $instance) {
			$removeChild = true;
			$child = new Resource(new \common\NamedArguments(array('primaryKey' => $instance->resourceID)));
			...
			foreach ($parents as $pinstance) {
				$parentResourceObj = new Resource(new \common\NamedArguments(array('primaryKey' => $pinstance->relatedResourceID)));
	...
	public function getGeneralDetailSubjectLinkID() {
		...
		foreach ($result as $row) {
			$object = new GeneralDetailSubjectLink(new \common\NamedArguments(array('primaryKey' => $row['generalDetailSubjectLinkID'])));
			array_push($objects, $object);
		}
	...
	public function getDetailedSubjects($resourceID, $generalSubjectID) {
		...
		foreach ($result as $row) {
			$object = new DetailedSubject(new \common\NamedArguments(array('primaryKey' => $row['detailedSubjectID'])));
			array_push($objects, $object);
		}
