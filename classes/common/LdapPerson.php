...
class LdapPerson extends DynamicObject {
	public function __construct($userKey) {
		...
		if ($hostInfo) {
			...
			$connection = ldap_connect($hostInfo);

			//may need ldap_bind( $ds, $username, $password )
            $errorMessage = _("Could not connect to ") . $hostInfo;
			$boundConnection = ldap_bind($connection) or die("<br /><h3>{$errorMessage}</h3>");
			if ($boundConnection){
				$filter = "{$ldapConfig->search_key}={$userKey}";

				$searchResults = ldap_search($connection, $ldapConfig->base_dn, $filter);

				if ($entries = ldap_get_entries($connection, $searchResults)) {
					$entry = $entries[0];
					...
					foreach ($fieldNames as $fieldName) {
						...
						$this->$fieldName = $entry[$ldapConfig->$configName][0];
