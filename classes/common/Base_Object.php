...
class Base_Object {
	...
	public function __construct(NamedArguments $arguments = NULL) {
		if (method_exists($this, 'init')) {
			if (!is_a($arguments, 'common\NamedArguments')) {
	...
	//Slight revision to PHP's default get_class() so that it will ignore Namespaces.
	protected function getClassName($class){
		$className = get_class($class);
		$lastSlashes = strrpos($className, '\\');
		$output = ($lastSlashes > 0) ? substr($className, $lastSlashes+1) : $className;
		return $output;
	}
