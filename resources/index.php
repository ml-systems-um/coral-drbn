...
**************************************************************************************************************************
*/
use common\{CoralSession, Configuration, Html};
use resources\controllers\{Resource};
use resources\models\{Fund};
		...
		<main id="main-content">
			...
			<aside id="side" class="block-form" role="search">
				<form method="get" action="ajax_htmldata.php?action=getSearchResources" id="resourceSearchForm">
					<?php
						$hiddenElements = ["orderBy", "page", "recordsPerPage", "startWith"];
						foreach($hiddenElements as $hiddenName){
							$hiddenValue = ($search[$hiddenName]) ?? '';
							echo (new Html())->hidden_search_field_tag($hiddenName, $hiddenValue);
						}
					?>
					...
					<p class='searchRow'>
						...
						<select name='search[fund]' id='searchFund' class ='changeInput'>
							...
							<?php
								$searchedFund = ($search['fund']) ?? FALSE;
								$noneIsSelected = ($searchedFund == "none") ? "selected" : "";
								echo "<option value='none' {$noneIsSelected}>"._("(none)")."</option>";

								$fundType = new Fund();
								foreach($fundType->allAsArray() as $fund) {
									$selected = ($fund['fundID'] == $search['fund']) ? "selected" : "";
									...
									echo "<option value='{$fund['fundID']}' {$selected}>{$fundName} [{$fund['fundCode']}]</option>";
								}
							?>
						</select>
					</p>
						...
						<select name='search[acquisitionTypeID]' id='searchAcquisitionTypeID'>
							...
							<?php
								...
								foreach($acquisitionType->allAsArray() as $display) {
									$selected = ($search['acquisitionTypeID'] == $display['acquisitionTypeID']) ? "selected" : "";
							...
							<?php
								...
								foreach($status->allAsArray() as $display) {
									...
									if (strtoupper($display['shortName']) != 'SAVED'){
										$selected = ($search['statusID'] == $display['statusID']) ? "selected" : "";
